...
public class Utility {
	...
	public String modifyEditorTxt(String text) {
		...
		text = m.replaceAll("http://www.crew-gs.com");
	...
	public String getKeyword(String str) {
		...
		for (int i = 0; i < array.length; i++) {				
			// 데스크탑버전 네이버 키워드광고
			if (array[i].indexOf("n_keyword=") > -1 ) { 
			...
			}else if (array[i].indexOf("query=") > -1 && array[i].indexOf("oquery=") < 0) {
				keyword = array[i].toString();
				keyword = keyword.substring(keyword.indexOf("=") + 1, keyword.length());
			} else if (array[i].indexOf("q=") > -1 && array[i].indexOf("oq=") < 0) {
				keyword = array[i].toString();
				keyword = keyword.substring(keyword.indexOf("=") + 1, keyword.length());
			} else {
				//keyword = visiter_refer;
			}
	...
	public String getSector(String str, String current) throws MalformedURLException {
		...
		else if(str.indexOf("tstory") > -1){ sector = "tstory"; }
		else {
